...
class HomeView(ListView):
    ...
    def get_queryset(self):
        ...
        return Listing.objects.filter(status=Listing.Status.ACTIVE).order_by('-is_promoted', '-created_at')[:8]
...
class ListingDetailView(DetailView):
    ...
    def get_object(self, queryset=None):
        # Allow owner, accountant, moderator, or admin to view non-active listings too
...
def seller_profile(request, username):
    ...
    listings = Listing.objects.filter(seller=seller, status=Listing.Status.ACTIVE).order_by('-is_promoted', '-created_at')
